...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(tabs)` | `/_sitemap` | `/history` | `/home` | `/practice` | `/settings` | `/study`;
  type DynamicRoutes<T extends string> = `/history/${SingleRoutePart<T>}`;
  type DynamicRouteTemplate = `/history/[id]`;
